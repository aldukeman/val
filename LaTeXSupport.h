...
namespace VAL
{
  struct showList
  {
  	void operator()(const pair<double, std::vector<std::string> > & ps) const
  	{
  		if(LaTeX)
  		{		
  			std::string s;
  			*report << ps.first<<" \\>";
  			for(std::vector<std::string>::const_iterator i = ps.second.begin(); i != ps.second.end() ; ++i)
  			{
  				s = *i; 
  				replaceSubStrings(s,"/","/\\-");
              	latexString(s);
  		...
  		{
  			cout << "\nValue: " << ps.first << "\n ";
  			copy(ps.second.begin(), ps.second.end(), std::ostream_iterator<std::string>(cout," "));
  			cout << "\n";
  		}
  	}
  };
  
  void displayFailedLaTeXList(std::vector<std::string> & vs);
  ...
  private:
  	...
  	int noGCPages;
  	int noGCPageRows;
  	std::vector<std::string> ganttObjectsAndTypes;
  	std::vector<std::string> ganttObjects;
  public:
