...
namespace VAL
{
  ...
  public:
    Validator(const DerivationRules* dr, double tol, TypeChecker& tc, const operator_list* ops,
      ...
      fef(), pf(this), errorLog(), derivRules(dr), events(ops), tolerance(tol), typeC(tc),
      metric(m), stepcount(p->size()), stepLength(lengthDefault), durative(isDur),
      invariantWarnings(), actionRegistry(), graphs(), gantt(), theplan(this,ops,p),
      state(this,is), finalInterestingState(0), followUp(theplan.end()), thisStep(theplan.begin()),
      tjm(this,cg1,cg2)
    {
    ...
    void reset(const plan* p, const operator_list* ops, con_goal* cg1, con_goal* cg2, const effect_lists* is);
  ...
  public:
    PlanRepair(vector<plan_step*> initLits, double dl, const DerivationRules* dr, double tol,
      ...
      analysis* ca) :
      p(p2), timedInitialLiteralActions(initLits), deadLine(dl), v(dr, tol, tc, ops, is, p1, m,
        lengthDefault, isDur, ca->the_domain->constraints, ca->the_problem->constraints),
      typeC(tc), metric(m), stepLength(lengthDefault), durative(isDur), operators(ops),
      initialState(is), theGoal(g)
      {}
