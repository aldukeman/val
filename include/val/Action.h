...
namespace VAL
{
  ...
  protected:
    ...
    struct ActionParametersOutput
    {
      ...
      string operator()(const var_symbol * v) const { return bindings.find(v)->second->getName(); }
    };
  ...
  public:
    ...
    virtual bool constructEffects(Ownership& o, EffectsRecord& e, const State* s, bool markPreCons) 
      const;
  ...
  public:
    CondCommunicationAction(Validator* v, const durative_action* a, const const_symbol_list* bs,
      goal_list* gs, goal_list* gi, goal_list* ge, effect_lists* es, effect_lists* el);
    CondCommunicationAction(Validator* v, const durative_action* a, const const_symbol_list* bs,
      goal_list* gs, goal_list* gi, goal_list* ge, effect_lists* es, effect_lists* el,
      Environment* vs);
  ...
  void buildForAllCondActions(Validator* vld, const durative_action* da,
    const const_symbol_list* params, goal_list* gls, goal_list* gli, goal_list* gle, 
    effect_lists* locels, effect_lists* locele, const var_symbol_list* vars,
    var_symbol_list::const_iterator i, vector<const CondCommunicationAction*>& condActions,
    Environment* env);
  ...
  public:
    DurativeActionElement(Validator* v, const action* a, const const_symbol_list* bs, 
      double d, const goal_list* ds, const InvariantAction * inv, const CtsEffectAction* ctsEff,
      const vector<const CondCommunicationAction*>& cas, const plan_step * ps = 0) :
      Action(v, a, bs, ps), duration(d), durs(ds), invariant(inv), ctsEffects(ctsEff), 
      condActions(cas)
    {
      ...
      DurativeActionElement(v, a, bs, d, ds, 
        (inv->getGoals()->empty()) ? 0 :  //do not create invariant action if no invariants to check!
          new InvariantAction(v,this,
            new safeaction(a->name, a->parameters, const_cast<conj_goal *>(inv), new effect_lists(),
              a->symtab),
            bs, ps),
        //similarly do not create cts effect action if no cts effects!
        (elc->assign_effects.empty() && elc->forall_effects.empty() && ccas.empty()) ? 0 :
