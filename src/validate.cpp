...
void usage()
{
  cout  << "VAL: The PDDL+ plan validation tool\n"
        ...
        << "    -r <n> <p> <m>  - Analyse the plan for its robustness, each action timestamp to within\n"
        << "                      a (float) value of n, each PNE to within a (float value) of p, for m\n"
        << "                      test plans.\n"
        << "    -ra <p>         - Calculate robustness of plan with respect to varying action\n"
        << "                      timestamps, whilst varying PNEs to within a (float value) of\n"
        << "                      p (default p = 0).\n"
  ...
  try
  {
    ...
    if(Robust)
    {
      analysePlansForRobustness(argc, argv, argcount, tc, derivRules, tolerance, lengthDefault, 
        giveAdvice, robustMeasure, noTestPlans, calculateActionRobustness, calculatePNERobustness, 
        robustMetric, robustDist);
    }
    else     
